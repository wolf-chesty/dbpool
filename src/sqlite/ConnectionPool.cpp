...
void ConnectionPool::initialize_sqlite()
{
...
void ConnectionPool::shutdown_sqlite()
{
    std::unique_lock const lock(sqlite_init_mutex_);
    if (--sqlite_use_count_ == 0) {
#ifdef NDEBUG
                                         ...
                                         [[maybe_unused]] size_t threshold)
{
    ...
    do {
        std::unique_lock lk(optimization_mutex_);
        optimization_cv_.wait_for(lk, period, [this]() -> bool { return !run_optimization_thread_; });
...
{
    std::scoped_lock lk(optimization_thread_mutex_);

    if (period.count() > 0 && !optimization_thread_) {
        sqlite3_stmt *stmt{};
