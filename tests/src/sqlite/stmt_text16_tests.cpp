...
TEST(SQLite3Test, text16_bind_test)
{
    ...
    EXPECT_EQ(val, query_stmt.get_text16(0));
    EXPECT_EQ(dbpool::PreparedStmt::ReturnCode::done, query_stmt.execute());

    EXPECT_THROW(query_stmt.get_text16(-1), std::runtime_error);
    EXPECT_THROW(query_stmt.get_text16(1), std::runtime_error);
}

TEST(SQLite3Test, invalid_text16_bind_test)
{
    dbpool::sqlite::ConnectionPool db_pool(":memory:");
    ...
    auto insert_stmt = conn.get_stmt("INSERT INTO t1 (id, test_val) VALUES (?, ?)");
    EXPECT_THROW(insert_stmt.bind_text16(0, u"Test"), std::runtime_error);
    insert_stmt.reset();
    EXPECT_THROW(insert_stmt.bind_text16(3, u"Test"), std::runtime_error);
}

TEST(SQLite3Test, empty_text16_bind_test)
{
    dbpool::sqlite::ConnectionPool db_pool(":memory:");
    ...
    EXPECT_THROW(query_stmt.get_text16(0), std::runtime_error);
}

TEST(SQLite3Test, null_text16_bind_test)
{
    dbpool::sqlite::ConnectionPool db_pool(":memory:");
